...
def process(filename, minutes):
    ...
    def try_elephant(minute, valve, valvesopen, elephantvalvesopen, path, prevflow):
        ...
        if valvesleft == 0:
            ...
            #print('E', prevflow + pressure_released, path)
            ...
            for v in tunnels[valve]:
                if not_dupe(path, valve, v, minute+1, valvesopen):
                    if minute > 3:
                        score = check_score(1, minute+1, v, valvesopen)
                        if score > maxscore:
                            maxscore = score
                            maxvalve = v
                    else:
            ...
            if maxvalve != None:
                ...
                testval = try_elephant(minute+1, maxvalve, valvesopen, elephantvalvesopen, path+' -> '+maxvalve, prevflow+pressure_released)
                if testval > maxval:
                    maxval = testval
            #if maxval == termval:
                #print('E', prevflow + pressure_released + maxval, path)
            ...
            for v in tunnels[valve]:
                if not_dupe(path, valve, v, minute+1, valvesopen):
                    ...
                    if score > maxscore:
                        ...
                        maxvalve = v
            if maxvalve != None:
                testval = try_valve(minute+1, maxvalve, valvesopen, path+' -> '+maxvalve, prevflow+pressure_released)
                if testval > maxval:
                    maxval = testval
