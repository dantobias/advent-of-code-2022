def process(filename):
    ...
    while not (destrow, destcol) in poslist:
        ...
        for m in moves:
            mdist = abs(m[0]-destrow) + abs(m[1] - destcol)
            if mdist < mindist:
                mindist = mdist

        for m in moves:
            #mdist = abs(m[0]-destrow) + abs(m[1] - destcol)
            if (not m in newposlist): # and mdist < mindist + 20:
                newposlist.append(m)
        ...
        result += 1
        if result % 10 == 0:
            print(result, len(poslist), mindist)

    print('First crossing', result)

    # Go back
    poslist = [(destrow, destcol)]
    while not (startrow, startcol) in poslist:
        advance_blizzards()
        ...
        for m in moves:
            mdist = abs(m[0]-startrow) + abs(m[1] - startcol)
            if mdist < mindist:
                mindist = mdist
